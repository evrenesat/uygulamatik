...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Event', fields ['node']
        db.delete_unique('event', ['node_id'])


        # Changing field 'Event.node'
        db.alter_column('event', 'node_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['umatik.Node'], null=True))
        # Deleting field 'Sponsor.node'
        db.delete_column(u'umatik_sponsor', 'node_id')


    def backwards(self, orm):

        # Changing field 'Event.node'
        db.alter_column('event', 'node_id', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['umatik.Node'], unique=True, null=True))
        # Adding unique constraint on 'Event', fields ['node']
        db.create_unique('event', ['node_id'])

        # Adding field 'Sponsor.node'
        db.add_column(u'umatik_sponsor', 'node',
                      self.gf('django.db.models.fields.related.OneToOneField')(to=orm['umatik.Node'], unique=True, null=True, blank=True),
                      keep_default=False)
