...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Profile', fields ['place', 'user']
        db.delete_unique(u'umatik_profile', ['place_id', 'user_id'])

        # Deleting field 'Profile.app'
        db.delete_column(u'umatik_profile', 'app_id')

        # Deleting field 'Profile.place'
        db.delete_column(u'umatik_profile', 'place_id')

        # Adding field 'Profile.node'
        db.add_column(u'umatik_profile', 'node',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['umatik.Node'], null=True, blank=True),
                      keep_default=False)

        # Adding M2M table for field apps on 'Profile'
        db.create_table(u'umatik_profile_apps', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('profile', models.ForeignKey(orm[u'umatik.profile'], null=False)),
            ('application', models.ForeignKey(orm[u'umatik.application'], null=False))
        ))
        db.create_unique(u'umatik_profile_apps', ['profile_id', 'application_id'])


        # Changing field 'Profile.user'
        db.alter_column(u'umatik_profile', 'user_id', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['auth.User'], unique=True))
        # Adding unique constraint on 'Profile', fields ['node', 'user']
        db.create_unique(u'umatik_profile', ['node_id', 'user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Profile', fields ['node', 'user']
        db.delete_unique(u'umatik_profile', ['node_id', 'user_id'])


        # User chose to not deal with backwards NULL issues for 'Profile.app'
        raise RuntimeError("Cannot reverse this migration. 'Profile.app' and its values cannot be restored.")
        # Adding field 'Profile.place'
        db.add_column(u'umatik_profile', 'place',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['umatik.Place'], null=True, blank=True),
                      keep_default=False)

        # Deleting field 'Profile.node'
        db.delete_column(u'umatik_profile', 'node_id')

        # Removing M2M table for field apps on 'Profile'
        db.delete_table('umatik_profile_apps')


        # Changing field 'Profile.user'
        db.alter_column(u'umatik_profile', 'user_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'], unique=True))
        # Adding unique constraint on 'Profile', fields ['place', 'user']
        db.create_unique(u'umatik_profile', ['place_id', 'user_id'])
