...
class TabbedPageConfig(object):
    ...
    def __init__(self, request, model_admin, obj_or_id=None):
        ...
        if hasattr(self, 'SuperFieldsConfig'):
            for f in dir(self.SuperFieldsConfig):
                if f.startswith("_"):
                    continue
                else:
                    setattr(self.FieldsetsConfig, f, getattr(self.SuperFieldsConfig, f))
...
#            super(TabbedModelAdmin, self).declared_fieldsets = []
    ...
    def get_page_config(self, request, obj_or_id=None, **kwargs):
        ...
        """
#        assert 0, request.user.is_superuser
#            assert 0, self.page_config_class.TabsConfig.super_tab
#        if self._page_config is None:
        self._page_config = self.page_config_class(request, self, obj_or_id=obj_or_id)
        if not request.user.is_superuser and hasattr(self._page_config.Tabs, 'super_tab'):
            del self._page_config.Tabs.super_tab
        return self._page_config

    def get_fieldsets(self, request, obj=None):
        if not self.page_config_class:
            self.declared_fieldsets = super(TabbedModelAdmin, self)._declared_fieldsets()
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        if self.page_config_class:
            self.change_form_template = getattr(self, 'change_template', 'admin/tabbed_change_form.html')
        else:
            return super(TabbedModelAdmin, self).change_view(request, object_id, form_url, extra_context)
