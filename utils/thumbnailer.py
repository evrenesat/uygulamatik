...
def customThumbnailer(img, opts, id=None, mark=False, prefix=''):
    results = []
    try:
        if prefix:
            prefix += '_'
        ...
        for opt in opts:
            if not id:
                id = basename(splitext(img.name)[0])
            size, name = opt[:2], '%s%s_%s' % (prefix, opt[2], id)
            thumbnail_options = dict(size=size, upscale=False, custom_name=name) #crop=crop,
